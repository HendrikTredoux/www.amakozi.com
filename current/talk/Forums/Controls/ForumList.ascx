...
<table summary='<%# Resources.ForumResources.ForumsTableSummary %>'  cellpadding="0" cellspacing="1" border="0" width="100%">
   ...
   <asp:Repeater id="rptForums" runat="server" >
      ...
      <ItemTemplate >
         <tr class="modulerow">
            <td headers='<%# tdSubscribedHead.ClientID %>' id="tdSubscribed" runat="server" enableviewstate="false" class="txtmed padded fsubscribe" Visible='<%# Request.IsAuthenticated %>' > 
            ...
            <td headers='t2' class="ftitle"> 
				...
				<asp:HyperLink id="HyperLink3" runat="server" EnableViewState="false"
				    ...
				    NavigateUrl='<%# SiteRoot + "/Forums/RSS.aspx?ItemID=" + DataBinder.Eval(Container.DataItem,"ItemID") + "&mid=" + ModuleId + "&pageid=" + PageId.ToString() %>' 
				    Visible="<%# Config.EnableRSSAtForumLevel %>"  />
				<asp:HyperLink id="viewlink1" runat="server" SkinID="TitleLink" EnableViewState="false"
				    NavigateUrl='<%# SiteRoot + "/Forums/ForumView.aspx?pageid=" + PageId.ToString() + "&mid=" + ModuleId + "&ItemID=" + DataBinder.Eval(Container.DataItem,"ItemID")   %>'>
      ...
      <alternatingItemTemplate>
		<tr class="modulealtrow">
            <td headers='<%# tdSubscribedHead.ClientID %>' id="tdSubscribedAlt" runat="server" enableviewstate="false" class="txtmed padded fsubscribe" Visible='<%# Request.IsAuthenticated %>'>  
                <div id="divSbubcriberCount" runat="server" enableviewstate="false" visible='<%# (Config.ShowSubscriberCount &&(!IsEditable)) %>'>
                   <asp:Literal ID="litSubCount" runat="server" EnableViewState="false" Text='<%# FormatSubscriberCount(Convert.ToInt32(Eval("SubscriberCount")))%>' />
                </div> 
...
<div id="divEditSubscriptions" runat="server" enableviewstate="false" class="settingrow forumnotification">
    <asp:HyperLink ID="lnkModuleRSS" runat="server" EnableViewState="false" CssClass="forumfeed forummodulefeed" />
